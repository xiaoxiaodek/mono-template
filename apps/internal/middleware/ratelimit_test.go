...
func TestRateLimitKeySelection(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "global", key: GlobalRateLimitKey("control-api"), want: "global:control-api", ok: true},
...
func TestRedisFixedWindowRateLimiterExecutesAtomicScriptWithTTL(t *testing.T) {
	client := &fakeRedisEvaler{result: int64(1)}
	limiter := NewRedisFixedWindowRateLimiter(client, "ads", 7, 1500*time.Millisecond)
	...
	if len(client.args) != 2 || client.args[0] != int64(7) || client.args[1] != int64(1500) {
		t.Fatalf("args = %#v, want limit and TTL milliseconds", client.args)
	}
	if !strings.Contains(client.script, `redis.call("INCR", KEYS[1])`) ||
		!strings.Contains(client.script, `redis.call("PEXPIRE", KEYS[1], ARGV[2])`) {
		t.Fatalf("Lua script does not atomically increment and set TTL: %q", client.script)
	}
}

func TestRedisFixedWindowRateLimiterRepairsMissingTTL(t *testing.T) {
	client := &fakeRedisEvaler{result: int64(1)}
	limiter := NewRedisFixedWindowRateLimiter(client, "ads", 7, time.Second)

	if _, err := limiter.Allow(context.Background(), "user:usr_42"); err != nil {
		t.Fatalf("Allow() error = %v", err)
	}
	if !strings.Contains(client.script, `redis.call("PTTL", KEYS[1])`) {
		t.Fatalf("Lua script does not inspect an existing counter TTL: %q", client.script)
	}
	if !strings.Contains(client.script, `current == 1 or ttl < 0`) {
		t.Fatalf("Lua script does not repair a missing TTL: %q", client.script)
	}
}

func TestNewRedisFixedWindowRateLimiterRejectsLimitAboveLuaSafeInteger(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("constructor did not reject a limit above Lua's safe integer range")
		}
	}()

	NewRedisFixedWindowRateLimiter(
		&fakeRedisEvaler{result: int64(1)},
		"ads",
...
func TestRedisFixedWindowRateLimiterDeniesAfterLimit(t *testing.T) {
	limiter := NewRedisFixedWindowRateLimiter(&fakeRedisEvaler{result: int64(0)}, "ads", 1, time.Second)
...
func TestRedisFixedWindowRateLimiterSurfacesRedisErrors(t *testing.T) {
	want := errors.New("redis unavailable")
	limiter := NewRedisFixedWindowRateLimiter(&fakeRedisEvaler{err: want}, "ads", 1, time.Second)
