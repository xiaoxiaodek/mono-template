...
func (c Config) Validate() error {
	...
	if c.App.Env == "prod" {
		...
		if c.Observability.PprofEnabled {
			return errors.New("prod pprof must be disabled")
		}
		if c.Observability.SwaggerEnabled {
			return errors.New("prod swagger must be disabled")
		}
	}
	if c.Observability.Tracing.Enabled {
		...
		if c.Observability.Tracing.SamplingRatio < 0 || c.Observability.Tracing.SamplingRatio > 1 {
			return errors.New("tracing sampling ratio must be within [0, 1]")
		}
	}
...
type rawConfig struct {
	...
	Redis struct {
		...
		RateLimitFailOpen bool   `yaml:"ratelimit_fail_open"`
	} `yaml:"redis"`
...
func (r rawConfig) toConfig() Config {
	return Config{
		...
		CORS: CORSConfig{
			...
			AllowedMethods:   r.CORS.AllowedMethods,
			AllowedHeaders:   r.CORS.AllowedHeaders,
			AllowCredentials: r.CORS.AllowCredentials,
			MaxAge:           time.Duration(r.CORS.MaxAge),
		},
		Database: DatabaseConfig{
		...
		Redis: RedisConfig{
			...
			RateLimitFailOpen: r.Redis.RateLimitFailOpen,
		},
		Observability: ObservabilityConfig{
			PprofEnabled:   r.Observability.PprofEnabled,
			SwaggerEnabled: r.Observability.SwaggerEnabled,
			Tracing: TracingConfig{
				Enabled:       r.Observability.Tracing.Enabled,
				Endpoint:      r.Observability.Tracing.Endpoint,
				SamplingRatio: r.Observability.Tracing.SamplingRatio,
			},
		},
	}
}
